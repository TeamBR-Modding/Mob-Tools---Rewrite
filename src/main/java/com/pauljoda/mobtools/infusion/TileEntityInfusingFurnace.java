		...
		for(int horiz = -2; horiz <= 2; horiz++)    // Horizontal (X or Z)
		{
			for(int vert = -1; vert <= 3; vert++)   // Vertical (Y)
			{
				for(int depth = -2; depth <= 2; depth++) // Depth (Z or X)
				{
					...
					if(vert == -1)
					{
						if(blockId.getUnlocalizedName().equals(Blocks.soul_sand.getUnlocalizedName()))
							output = output - 300;
					}
					else
					{
						if(blockId.getUnlocalizedName().equals(Blocks.bookshelf.getUnlocalizedName()))
							output =  output - 300;

						if(blockId.getUnlocalizedName().equals(Blocks.skull.getUnlocalizedName()))
							output = output - 6000;
