		...
		for (EntityPlayer player : getPlayersOnGrid()) 
		{
			if(player.isSneaking())
			{
				for(int i = 1; i <= GeneralSettings.maxEnderPadDistance; i++)
				{
					...
					if(world.getBlockId(xPos, yPos, zPos) == ToolManager.enderPad.blockID)
					{
						if(world.getBlockTileEntity(xPos, yPos, zPos) instanceof TileEntityEnderPad)
						{
							TileEntityEnderPad enderPad = (TileEntityEnderPad)world.getBlockTileEntity(xPos, yPos, zPos);
