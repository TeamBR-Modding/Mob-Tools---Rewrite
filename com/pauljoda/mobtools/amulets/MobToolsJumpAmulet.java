...
public class MobToolsJumpAmulet extends MobToolsAmulet {
	...
	int cooldown;
	public MobToolsJumpAmulet(int par1) {
		super(par1);
		...
		if(player.isSneaking())
		{
			if(itemstack.stackTagCompound == null)
			{
				...
				if(!par2World.isRemote)
					player.addChatMessage(EnumChatFormatting.GREEN + "Jump Boost Enabled");
			}
			else
			{
				if(itemstack.stackTagCompound.getBoolean("Active"))
				{
					...
					if(!par2World.isRemote)
						player.addChatMessage(EnumChatFormatting.RED + "Jump Boost Disabled");
				}
				else
				{
					itemstack.stackTagCompound.setBoolean("Active", true);
